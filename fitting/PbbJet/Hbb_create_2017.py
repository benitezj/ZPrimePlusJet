...
from sampleContainer_2017 import *
...
def main(options, args):
    ...
    if options.data:
        if options.muonCR:
            container = sampleContainer('data_obs', files, 1, options.dbtagmin, options.dbtag, 
                                        options.lumi, True, False,
                                        '((triggerBits&4)&&passJson)', False)
        else:
            container = sampleContainer('data_obs', files, 1, options.dbtagmin, options.dbtag, 
                                        options.lumi, True, False,
                                         '((triggerBits&32768 || triggerBits&134217728 || triggerBits&4096 || triggerBits&8192 || triggerBits&16384 || triggerBits&16 || triggerBits&16777216 || triggerBits&2097152)&&passJson)', False)
    else:
        container = sampleContainer(options.process,files, 1, options.dbtagmin, options.dbtag, 
                                    options.lumi, False, False,
                                    '1', False)
    ...
    if options.muonCR:
        ...
        plots = ['h_msd_v_pt_ak8_topR6_N2_pass', 'h_msd_v_pt_ak8_topR6_N2_fail',
                 ...
                 'h_eta_ak8',
                 'h_ht',
                 'h_dbtag_ak8_aftercut',
                 'h_n2b1sdddt_ak8_aftercut',
                 'h_rho_ak8']
